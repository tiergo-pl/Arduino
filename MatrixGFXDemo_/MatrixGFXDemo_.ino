...
void count_pixels()
{
	...
	for (uint16_t i = 0; i < mh; i++)
	{
		...
		{
			matrix->drawPixel(j, i, i % 3 == 0 ? LED_BLUE_HIGH : i % 3 == 1 ? LED_RED_HIGH
																																			: LED_GREEN_HIGH);
...
void display_circles()
{
	...
	if (min(mw, mh) > 12)
		matrix->drawCircle(mw / 2 - 1, mh / 2 - 1, min(mh / 2 - 1, mw / 2 - 1), LED_CYAN_HIGH);
	matrix->show();
}
...
{
	uint8_t size = max(int(mw / 16), 1);
	matrix->clear();
	matrix->setTextWrap(false); // we don't wrap text so it scrolls nicely
	matrix->setTextSize(1);
	matrix->setRotation(0);
	for (int8_t x = 7; x >= -127; x--) // -127 is minimum proper value, longer string lowe value needed
	{
		matrix->clear();
		matrix->setCursor(x, 0);
		matrix->setTextColor(LED_GREEN_HIGH);
		matrix->print("Hello, Adafruit_NeoPixel");
		if (mh > 11)
		{
...
void display_panOrBounceBitmap(uint8_t bitmapSize, uint8_t bitmapSize_h = 0)
{
	if (!bitmapSize_h)
	{
		bitmapSize_h = bitmapSize;
	}

	// keep integer math, deal with values 16 times too big
	// start by showing upper left of big bitmap or centering if the display is big
	int16_t xf = max(0, (mw - bitmapSize) / 2) << 4;
	int16_t yf = max(0, (mh - bitmapSize_h) / 2) << 4;
	...
	for (uint16_t i = 1; i < 200; i++)
	{
		...
		// pan 24x24 pixmap
		if (bitmapSize > 8)
			matrix->drawRGBBitmap(x, y, (const uint16_t *)bitmap24, bitmapSize, bitmapSize_h);
#ifdef BM32
		if (bitmapSize == 32)
			matrix->drawRGBBitmap(x, y, (const uint16_t *)bitmap32, bitmapSize, bitmapSize_h);
#endif
		matrix->show();
...
#endif
	// pan a big pixmap
	bitmap24 = Heart::bitmap24;
	display_panOrBounceBitmap(24);
	bitmap24 = Smiley::bitmap24;
	display_panOrBounceBitmap(24);
	bitmap24 = Rinky;
	display_panOrBounceBitmap(32, 16);
	bitmap24 = rainbow32;
	display_panOrBounceBitmap(32);
	// bitmap24 = rainbow24;
	// display_panOrBounceBitmap(24);
	bitmap24 = spectrum;
	display_panOrBounceBitmap(32, 8);
	// bitmap24 = Google::bitmap32;
	// display_panOrBounceBitmap(32);
	// bitmap24 = Linux::bitmap32;
	// display_panOrBounceBitmap(32);
